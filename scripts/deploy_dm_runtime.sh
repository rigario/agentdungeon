...
if [[ "$VERIFY_ONLY" != "1" ]]; then
  ...
  ssh "$DEPLOY_HOST" "sudo mkdir -p '$DEPLOY_APP_DIR/scripts' && sudo cp /tmp/check_deployment_parity.py '$DEPLOY_APP_DIR/scripts/check_deployment_parity.py' && owner_group=\$(id -un):\$(id -gn) && sudo chown \"\$owner_group\" '$DEPLOY_APP_DIR/scripts/check_deployment_parity.py' || true"
